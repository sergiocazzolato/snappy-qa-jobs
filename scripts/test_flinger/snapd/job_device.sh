...
test_data:
    test_cmds: |
        ...
        . $JOBS_PROJECT/scripts/utils/run_setup.sh "{device_ip}" "$DEVICE_PORT" "$PROJECT" "$SPREAD_SETUP" "$SPREAD_ENV" "$SPREAD_PARAMS"
        . $JOBS_PROJECT/scripts/utils/run_tests.sh "{device_ip}" "$DEVICE_PORT" "$PROJECT" "$SPREAD_TESTS" "$SPREAD_ENV" "$SKIP_TESTS" "$SPREAD_PARAMS"
EOF
