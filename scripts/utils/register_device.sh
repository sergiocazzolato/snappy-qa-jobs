...
SCRIPT="( snap list core &>/dev/null && sudo snap install jq ) || (snap list core18 &>/dev/null && sudo snap install jq-core18 && sudo snap alias jq-core18.jq jq) || (snap list core20 &>/dev/null && sudo snap install jq-core20 && sudo snap alias jq-core20.jq jq); \
